...
$(document).on('focus', '.autocomplete_rental', function() {
  ...
  $(this).autocomplete({
    ...
    minLength: 1,
    select: function(event, ui) {
      let x = getElementNumber($(this).attr('name'));
...
$('.flatpickr').flatpickr({
  ...
  onChange: function(selectedDates, dateStr, instance) {
    let row = getElementNumber($(instance.input).attr('name'));
...
{
    let departure_date = parseDate($('#rental_'+x+'_departure_date').val());
    let return_date = parseDate($('#rental_'+x+'_return_date').val());

    let diffDays = Math.ceil(Math.abs(return_date.getTime() - departure_date.getTime()) / (1000 * 3600 * 24));
    if(diffDays == 0)
    diffDays++;
...
$(document).on('focusout', '.updateOrder', function() {
  let type = getElementType($(this).attr('name'));
  let row = getElementNumber($(this).attr('name'));
